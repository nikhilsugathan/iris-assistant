        ...
        with self._playback_lock:
            ...
            try:
                ...
                for start in range(0, len(audio_array), chunk_size):
                    ...
                    stream.write(chunk)
                if not interrupted:
                    # Let the OS audio buffer drain before closing the stream.
                    # stream.write() fills the ring buffer but doesn't wait for
                    # the speaker to finish playing. stream.stop() aborts
                    # immediately, clipping the tail. sleeping for output
                    # latency + one chunk duration ensures clean playback end.
                    import time as _time
                    drain_s = getattr(stream, "latency", 0.1) + chunk_size / target_rate
                    _time.sleep(drain_s)
            finally:
                self._active_stream = None
                try:
