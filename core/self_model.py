...
class SelfModel:
    ...
    # Re-entrant lock — safe for nested calls from the same thread.
    # Guards all state mutations against races between the main loop and
    # daemon threads (greeting pre-gen at iris-greeting-gen, session-learning).
    ...
    def reset(self) -> None:
        """Atomically revert all session-scoped state to startup defaults.

        Call this alongside every brain.memory.conversation.clear() so that
        emotional_load, initiative_budget, and admin_unlocked cannot bleed
        across session resets or RBAC mode switches.
        """
        with self._lock:
            ...
            self.caution = 0.42
            self.warmth = 0.44
            self.initiative_budget = 0.50
            self.emotional_load = 0.20
            self.cognitive_mode = "idle"
            self.last_reason = "startup"
            self.active_roles = []
            self.recent_friction = []
    ...
    def note_success(self, summary: str) -> None:
        with self._lock:
            ...
            self.recent_friction = [
                item for item in self.recent_friction if item != summary[:80]
            ][-6:]
