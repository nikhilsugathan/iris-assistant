...
# ── Safe write roots — all file/folder creation is anchored here ──
# PROJECT_ROOT is the directory containing this file's parent (i.e. repo root).
_PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
_HOME = os.path.expanduser("~")
_SAFE_WRITE_ROOTS = [
    os.path.realpath(os.path.join(_HOME, "Desktop")),
    os.path.realpath(os.path.join(_HOME, "OneDrive", "Desktop")),
    os.path.realpath(os.path.join(_HOME, "Documents")),
    os.path.realpath(os.path.join(_HOME, "Downloads")),
    os.path.realpath(os.path.join(_HOME, "Music")),
    os.path.realpath(os.path.join(_HOME, "Pictures")),
    os.path.realpath(os.path.join(_HOME, "Videos")),
...
class ActionExecutor:
    ...
    def __init__(self, voice, brain):
        ...
        self.log_file    = getattr(Config, "ACTION_LOG_FILE", "iris_actions.log")
    ...
    # ─────────────────────────────────────────────────────────────
    # SECURITY: Safe path assertion
    # ─────────────────────────────────────────────────────────────

    def _assert_safe_path(self, filepath: str) -> str:
        """
        Resolve the canonical real path and verify it sits inside one of
        the allowed safe roots.  Raises ValueError on path traversal attempts.
        Returns the resolved absolute path on success.
        """
        real = os.path.realpath(os.path.abspath(filepath))
        for root in _SAFE_WRITE_ROOTS:
            if real == root or real.startswith(root + os.sep):
                return real
        raise ValueError(
            f"Path traversal blocked: '{filepath}' resolves outside the "
            f"allowed write zone. Action cancelled."
        )
    ...
    def handle_followup_response(self, user_input: str) -> str:
        ...
        if any(word in text for word in YES_WORDS):
            ...
            if action == "open_file":
                ...
                try:
                    # HARDENED: list-form Popen, no shell=True
    ...
    def _pattern_match(self, user_input: str) -> Optional[dict]:
        ...
        if open_match:
            ...
            return {
                ...
                "app_name": app,
                "command": cmd,   # store the bare executable name, not a shell string
                "is_dangerous": False
            }
...
Respond with ONLY the JSON. No explanation."""
    ...
    def _run_command(self, plan: dict) -> str:
        ...
        # ── HARDENED: Expand env vars before regex check ──────────────────────
        # Prevents %COMSPEC%, %WINDIR%, %SYSTEMROOT% from bypassing pattern match.
        expanded = os.path.expandvars(command)

        from core.security import BLOCKED_COMMANDS
        for target in (command, expanded):
        ...
        try:
            result = subprocess.run(
                command,
                shell=True,   # shell=True intentional here: _run_command is for explicit
                              # shell commands that have already passed the full security
                              # gate + defense-in-depth re-check above.
                capture_output=True,
                text=True,
                timeout=120,
...
Be specific and practical. No preamble."""
        ...
        response = self.brain._call_api(
            getattr(Config, "FALLBACK_BRAIN", "groq"), prompt,
            use_persona=False, use_memory=False
        )
    ...
    def _create_file(self, plan: dict) -> str:
        ...
        # ── HARDENED: assert path is within safe write zone ──────────────
        try:
            filepath = self._assert_safe_path(filepath)
        except ValueError as e:
            self._log(f"PATH TRAVERSAL BLOCKED (create_file): {filepath}")
            return str(e)
    ...
    def _create_folder(self, plan: dict) -> str:
        ...
        # ── HARDENED: assert path is within safe write zone ──────────────
        try:
            folder = self._assert_safe_path(folder)
        except ValueError as e:
            self._log(f"PATH TRAVERSAL BLOCKED (create_folder): {folder}")
            return str(e)
    ...
    def _open_app(self, plan: dict) -> str:
        """Open an application or URL — HARDENED: no shell=True string interpolation."""
        app     = (plan.get("app_name") or "").strip()
        command = (plan.get("command") or "").strip()
        url     = (plan.get("url") or "").strip()

        # Direct URL — open in browser (safe: webbrowser never touches shell)
        ...
        target = app_map.get(app.lower(), app) if app else command

        # If it resolved to a URL — use browser (no shell)
        if target and target.startswith("http"):
            return self.browser.open_url(target)

        # ── HARDENED: list-form Popen, shell=False ────────────────────────
        # "cmd /c start" is the Windows-safe equivalent of shell=True start.
        # The target/command is passed as a separate list element, never
        # interpolated into a shell string — prevents injection via app name.
        launch_target = target if target else command
        
        if launch_target:
            # ── ZERO-DAY PATCH: Block unquoted cmd.exe metacharacters ──
            if re.search(r'[&|;<>`\^]', launch_target):
                self._log(f"OPEN_APP BLOCKED — shell metachar in target: {repr(launch_target)}")
                return "Blocked — application name contains unsafe characters."
            ...
            try:
                ...
                self._log(f"OPENED: {launch_target}")
                return "Done."
            except Exception as e:
                return f"Couldn't open {app or launch_target}: {e}"
    ...
    def _write_to_file(self, plan: dict) -> str:
        ...
        # ── HARDENED: assert path is within safe write zone ──────────────
        try:
            safe_path = self._assert_safe_path(filename)
        except ValueError as e:
            self._log(f"PATH TRAVERSAL BLOCKED (write_to_file): {filename}")
            return str(e)

        if not os.path.exists(safe_path):
            return f"File '{os.path.basename(safe_path)}' doesn't exist. Use 'create file' first."

        with open(safe_path, "a", encoding="utf-8") as f:
            f.write(content + "\n")
        self._log(f"WROTE TO: {safe_path}")
        return "Done."
