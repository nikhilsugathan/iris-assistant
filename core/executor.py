...
class ActionExecutor:
    ...
    ACTION_TARGET_HINTS = (
        "file", "folder", "directory", "document", "note", "txt", "pdf",
        "script", "program", "app", "application", "browser", "window",
        "tab", "terminal", "powershell", "cmd", "command", "package",
        "python", "pip", "npm", "winget", "notepad", "calculator",
        "settings", "chrome", "edge", "explorer", "music", "youtube",
    )

    EXPLANATION_HINTS = (
        "how to", "how do i", "how can i", "want to know", "tell me about",
        "explain", "walk me through", "guide me through", "teach me",
        "what is", "what are", "why", "when", "where", "how much",
        "ingredients", "calories", "sugar", "recipe",
    )

    ACTION_PATTERNS = (
        r"^(?:please\s+)?(?:install|uninstall|download|setup|set up|delete|remove|clean|run|execute|launch|open|start|update|upgrade|configure|enable|disable|move|copy|rename)\b",
        r"^(?:please\s+)?(?:create|make|generate|write)\s+(?:a\s+|an\s+|the\s+|new\s+)?(?:file|folder|directory|document|note|script|project|playlist|draft)\b",
        r"^(?:can you|could you|would you|please|i need you to|help me)\s+(?:install|uninstall|download|setup|set up|create|make|generate|write|delete|remove|clean|run|execute|launch|open|start|update|upgrade|configure|enable|disable|move|copy|rename)\b",
        r"^(?:open|launch|start)\s+(?:chrome|edge|notepad|calculator|settings|browser|terminal|cmd|powershell|explorer)\b",
    )
    _GENERIC_ACTION_ONLY = {
        "install", "uninstall", "download", "setup", "set up", "delete", "remove",
        "clean", "run", "execute", "launch", "open", "start", "update", "upgrade",
        "configure", "enable", "disable", "move", "copy", "rename", "create", "make",
        "generate", "write",
    }
    _LEADING_FILLERS = ("yeah ", "yep ", "yes ", "ok ", "okay ", "sure ", "please ", "just ")

    def should_handle(self, user_input: str) -> bool:
        """Detect only explicit device / filesystem actions."""
        ...
        if stripped in self._GENERIC_ACTION_ONLY:
            return False
        stripped_tokens = stripped.split()
        if (
            len(stripped_tokens) <= 2
            and stripped_tokens
            and stripped_tokens[-1] in self._GENERIC_ACTION_ONLY
            and not any(target in stripped for target in self.ACTION_TARGET_HINTS)
        ):
            return False

        if any(hint in text for hint in self.EXPLANATION_HINTS):
            if not any(target in text for target in self.ACTION_TARGET_HINTS):
                return False

        if text.startswith(("search for ", "look up ", "google ", "search the web ", "browse for ")):
            return False

        return any(re.search(pattern, text, re.IGNORECASE) for pattern in self.ACTION_PATTERNS)
    ...
    def _pattern_match(self, user_input: str) -> Optional[dict]:
        ...
        # Captures: "play [song]", "play [song] on spotify/youtube", "search spotify for [song]"
        song_match = re.search(
            r"(?:play|stream|listen to|put on|search(?:\s+(?:spotify|youtube))?\s+for)\s+"
            r"(.+?)(?:\s+(?:on|from|via|using|in)\s+(spotify|youtube|yt|music))?$",
            user_input,
            re.IGNORECASE
        )
        if song_match:
            query = song_match.group(1).strip()
            platform_hint = (song_match.group(2) or "").lower().strip()
            for filler in ["me a song", "some music", "music", "something", "a song",
                           "me something", "something good"]:
                if query.lower() == filler:
                    query = ""
                    break
            # Determine platform: explicit hint > default to YouTube
            platform = "spotify" if platform_hint in ("spotify",) else "youtube"
            return {
                "action_type": "play_music",
                "description": f"play {query or 'music'} on {platform}",
...
Be specific and practical. No preamble."""
    ...
    def _play_music(self, plan: dict) -> str:
        """Search and open music on Spotify or YouTube.

        Strategy:
        - Spotify: try native URI protocol (opens Spotify app if installed),
                   fall back to Spotify web search URL.
        - YouTube: open YouTube search URL in default browser.
        """
        import webbrowser
        from urllib.parse import quote_plus

        raw_query = plan.get("search_query", "") or plan.get("description", "music")
        query = raw_query.replace("play ", "").strip() or "popular songs"
        platform = (plan.get("platform") or "youtube").lower()

        encoded = quote_plus(query)

        if platform == "spotify":
            # Try Spotify native URI first — opens the desktop/mobile app.
            try:
                import subprocess
    ...
    # Apps that have a native Windows URI protocol handler.
    # Try the URI first — Windows dispatches to the installed app if present,
    # falls through gracefully if not.  Fallback URL is the browser version.
    _APP_PROTOCOLS = {
    ...
    def _open_app(self, plan: dict) -> str:
        ...
        Priority order:
        1. Explicit URL in plan → browser directly
        2. Media/streaming apps → try native URI protocol, fallback to browser
        3. System app map → subprocess launch
        4. Raw command in plan → subprocess
        5. Raw target string → subprocess with 'start'
        """
        ...
        # 1. Explicit URL — open in browser immediately.
        ...
        # 2. Apps with native URI protocol + browser fallback.
        if app_key in self._APP_PROTOCOLS:
            proto, fallback_url = self._APP_PROTOCOLS[app_key]
            if proto:
                try:
                    # 'start "" "spotify:"' hands off to the installed app.
                    # This is silent — if the app isn't installed Windows ignores it.
                    subprocess.Popen(f'start "" "{proto}"', shell=True)
                    self._log(f"OPENED native app via protocol: {proto}")
                    return "Done."
                except Exception:
                    pass
            # Fallback: open browser version.
        ...
        target = app_map.get(app_key, app)

        # If target turned into an http URL (legacy fallback), use browser.
        if target and target.startswith("http"):
            webbrowser.open(target)
            self._log(f"OPENED URL: {target}")
            return "Done."

        # 4. Explicit command in plan.
        if command:
            try:
        ...
        # 5. Raw target — try to launch directly.
        if target:
            try:
