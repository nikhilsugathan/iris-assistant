...
def main() -> None:
    ...
    while True:
        try:
            ...
            try:
                # HARDENED: full hardware teardown, not just stop_speaking()
                # shutdown() stops the worker thread, joins it, and calls
                # pygame.mixer.quit() to release the OS audio device handle.
                voice.shutdown()
            except Exception:
                pass
            break
