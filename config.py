...
class Config:
    ...
    # ── Brain routing ────────────────────────────────────────────
    # PRIMARY_BRAIN and FALLBACK_BRAIN are intentionally read-only
    # class-level defaults here.  brain.py resolves the actual
    # live priority at __init__ time and stores it on self.primary_brain
    # / self.fallback_brain (instance variables) — it does NOT mutate
    # these class attributes.  This prevents thread-racing if multiple
    # Brain instances are ever spawned (e.g. in tests or parallel
    # sessions), because each instance carries its own resolved state
    # rather than overwriting a shared global.
    PRIMARY_BRAIN  = "ollama_fast"   # static default — do not reassign at class level
    FALLBACK_BRAIN = "ollama_smart"  # static default — do not reassign at class level
    ...
    # Measured noise floor on target hardware: ~4303 RMS units.
    # 4500 provides only a ~200-unit margin — too tight, causing
    # false triggers on fan noise and keyboard clicks.
    # 4900 gives a comfortable ~600-unit margin above the noise
    # floor while still reliably triggering on normal speech (~6000+).
    # Calibration via adjust_for_ambient_noise() will further refine
    # this at runtime; this value is the hard floor it starts from.
