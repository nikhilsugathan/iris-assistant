...
if (-not $Request) {
    $Request = Join-Path $ScriptDir "..\ai-collab\perplexity-bridge-request.md"
}

if (-not $Response) {
    $Response = Join-Path $ScriptDir "..\ai-collab\perplexity-bridge-response.md"
}

Write-Host ""
Write-Host "IRIS Perplexity Bridge" -ForegroundColor Cyan
...
$bridgeArgs = @(
    (Join-Path $ScriptDir "perplexity_bridge.py"),
    "--workflow", $Workflow,
    "--mode", $Mode,
